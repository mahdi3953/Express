...
router.post('/member/:id', auth, permit('student'), function (req, res) {
  Course.update({ _id: req.params.id }, { $push: { members: req.body.id } }).then(function (course) {
      User.update({ _id: req.body.id }, { $push: { Course: req.params.id } }).then(function (User) {
          res.send({
              status: 'success',
              data: { message: 'you got the course successfully' }
          })
      }).catch(function (err) {
          res.send({
              status: 'error',
